...
public class TEXT {
    ...
    // a method for counting frequency of each character of a sentence and storing
    // both the character and its frequency in a root of a new tree
    public void CouontLetters(String sentence) {
        ...
        for (int i = 0; i < sentence.length(); i++) {
            ...
            if (!IsCounted(sentence.charAt(i), sentence, i)) {
                ...
                // add the character and its frequency to a new tree (make it a root of a new
                // tree)
    ...
    // a method to mix all tree in the queue to one tree only the head of the queue
    // is needed
    public void MixTrees(NodeQueue head) {
        ...
        while (queue.head.next != null) {
            ...
            // in here we used treeRef variable to reach node refrecene to make this
            // connection between nodes inside the queue and new tree root like parent
            ...
            // upate the head to 2 next node of the queue to disconnect the first and second
            // node from the tree
            // garbage collection handle the deletion
            queue.head = queue.head.next.next;
            // adding the new tree we created inside the queue for to mix it also with
            // future trees
            queue.add(NewParent);
        }
    }
