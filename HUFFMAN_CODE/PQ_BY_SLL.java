        ...
        // so the frequency is not less or the head is null or both
            ...
            if (current != null) {
                ...
                if (current.next != null) {
                    // while the next of the current node not null and the frequency of the new node
                    // greater than nodes of the linked list
                    while (current.next != null && current.next.frequency <= tree.frequency) {
                        // go to next node
                        current = current.next;
                    }
                    // so now put the new node in last place either if we have a node greater than
                    // new node
...
class NodeQueue {
    // frequency and character of each node because we have node of tree inside
    // likned list node so we need
    // those variable for searching and puting tree nodes in correct place and
    // protect the order from low to high
    int frequency;
    // next of each node that tell us next node after current node
    NodeQueue next;
    // Idea of using this variable comes from Gemini AI to solve a problem in TEXT
    // class there is more detail in the TEXT class
    NodeTree treeRef;

    // constructor to intialize information of node of linked list with node of tree
    // informations
